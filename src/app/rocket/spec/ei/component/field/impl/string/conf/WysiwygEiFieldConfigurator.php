...
class WysiwygEiFieldConfigurator extends AdaptableEiFieldConfigurator {
	...
	public function __construct(WysiwygEiField $wysiwygEiField) {
		parent::__construct($wysiwygEiField);
		
		$this->autoRegister($wysiwygEiField);
	}
	...
	public function createMagDispatchable(N2nContext $n2nContext): MagDispatchable {
		...
		CastUtils::assertTrue($eiComponent instanceof WysiwygEiField);
	...
	public function setup(EiSetupProcess $eiSetupProcess) {
		...
		$wysiwygEiField = $this->eiComponent;
		CastUtils::assertTrue($wysiwygEiField instanceof WysiwygEiField);
		
		if ($this->attributes->contains(self::OPTION_MODE_KEY)) {
			try {
				$wysiwygEiField->setMode($this->attributes->get(self::OPTION_MODE_KEY));
		...
		if ($this->attributes->contains(self::OPTION_LINK_KEY)) {
			$wysiwygEiField->setLinkConfigClassNames((array) $this->attributes->get(self::OPTION_LINK_KEY));
		}
		
		if ($this->attributes->contains(self::OPTION_CSS_CONFIG_KEY)) {
			$wysiwygEiField->setCssConfigClassName($this->attributes->get(self::OPTION_CSS_CONFIG_KEY));			
		}
		
		if ($this->attributes->contains(self::OPTION_TABLE_EDITING_KEY)) {
			$wysiwygEiField->setTableEditingEnabled($this->attributes->get(self::OPTION_TABLE_EDITING_KEY));
		}
		
		if ($this->attributes->contains(self::OPTION_BBCODE_KEY)) {
			$wysiwygEiField->setBbcodeEnabled($this->attributes->get(self::OPTION_BBCODE_KEY));
		}
	}
}
