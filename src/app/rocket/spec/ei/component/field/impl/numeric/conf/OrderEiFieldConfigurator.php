...
class OrderEiFieldConfigurator extends NumericEiFieldConfigurator {
	...
	public function setup(EiSetupProcess $setupProcess) {
		...
		IllegalStateException::assertTrue($this->eiComponent instanceof OrderEiField);
		$eiDef = $setupProcess->getEiDef();
		
		if ($this->attributes->contains(self::OPTION_REFERENCE_FIELD_KEY)) {
			$this->eiComponent->setReferenceField($eiDef->getEiFieldCollection()->getById(
					$this->attributes->get(self::OPTION_REFERENCE_FIELD_KEY)));
		}
		
		$orderEiCommand = new OrderEiCommand();
		$orderEiCommand->setOrderEiField($this->eiComponent);
...
// 		}
	...
	private function generateReferenceEnumMags() {
		...
		foreach ($this->eiComponent->getEiSpec()->getEiFieldCollection()->combineAll() as $eiField) {
			if (!($eiField instanceof ConfEntityPropertyEiField)) continue;
			$referenceFields[$eiField->getId()] = $eiField->getLabelCode();
		}
