...
use rocket\spec\ei\component\field\impl\bool\command\OnlineEiCommand;
...
class OnlineEiFieldConfigurator extends AdaptableEiFieldConfigurator {
	const COMMON_ONLINE_PROP_NAME = 'online';
	
	public function __construct(OnlineEiField $onlineEiField) {
		parent::__construct($onlineEiField);

		$this->addMandatory = false;
		$this->addConstant = false;
		$this->autoRegister($onlineEiField);
	}
	...
	public function setup(EiSetupProcess $setupProcess) {
		...
		$onlineEiField = $this->eiComponent;
		IllegalStateException::assertTrue($onlineEiField instanceof OnlineEiField);
		
		$onlineEiCommand = new OnlineEiCommand();
		$onlineEiCommand->setOnlineEiField($onlineEiField);
