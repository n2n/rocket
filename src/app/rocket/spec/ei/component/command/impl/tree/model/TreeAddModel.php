...
class TreeAddModel extends AddModel {
	...
	public function setParentEntity(EiEntry $parentEiEntry) {
		return $this->parentEiEntry = $parentEiEntry;
	}
	/* (non-PHPdoc)
	...
	public function create(MessageContainer $messageContainer) {
		...
		if ($this->parentEiEntry !== null) {
			$parentEntity = $this->parentEiEntry->getLiveEntityObj();
		}
		...
		$entity = $eiMapping->getEiEntry()->getEntityObj();
		...
		return new EiEntry($eiFrame->getContextEiMask()->getEiEngine()->getEiSpec()->extractId($entity), $entity);
	}
}
