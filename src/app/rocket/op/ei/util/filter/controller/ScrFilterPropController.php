 ...
 */
namespace rocket\op\ei\util\filter\controller;
...
use rocket\op\ei\component\CritmodFactory;
use rocket\op\ei\util\filter\form\FilterPropItemForm;
use rocket\op\ei\manage\critmod\filter\data\FilterSetting;
...
class ScrFilterPropController extends ControllerAdapter implements ScrController {
	...
	public function doSimple(string $eiTypeId, ParamQuery $filterPropId, ParamQuery $propertyPath, ?string $eiMaskId = null) {
		...
		$filterDefinition = (new CritmodFactory($eiThing->getEiMask()->getEiPropCollection(), 
						$eiThing->getEiMask()->getEiModCollection()))
