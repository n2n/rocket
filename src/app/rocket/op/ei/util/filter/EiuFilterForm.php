...
class EiuFilterForm implements Dispatchable, UiComponent {
	...
	/**
	 * @return \rocket\op\ei\util\filter\controller\FilterJhtmlHook
	 */
	...
	/**
	 * @return \rocket\op\ei\manage\critmod\filter\data\FilterSettingGroup
	 */
	...
	function createView(?HtmlView $contextView = null) {
		if ($contextView !== null) {
			return $contextView->getImport('\rocket\op\ei\util\filter\view\eiuFilterForm.html',
		...
		return $viewFactory->create('rocket\op\ei\util\filter\view\eiuFilterForm.html', array('eiuFilterForm' => $this));
	}
