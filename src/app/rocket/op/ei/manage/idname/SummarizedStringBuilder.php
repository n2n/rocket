<?php
namespace rocket\op\ei\manage\idname;
...
class SummarizedStringBuilder {
	...
	static function detectUsedDefPropPaths(string $identityStringPattern, IdNameDefinition $idNameDefinition) {
		...
		foreach (array_keys($idNameDefinition->getAllIdNameProps()) as $defGuiPropPathStr) {
			if (false === strpos($identityStringPattern, self::createPlaceholderFromStr($defGuiPropPathStr))) {
				continue;
			}
	 ...
	 * @return string
	 */
	private static function createPlaceholderFromStr(string $defPropPathStr): string {
