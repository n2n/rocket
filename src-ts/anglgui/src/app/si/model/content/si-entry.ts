...
export class SiEntry {
	...
	get selectedTypeBuildup(): SiTypeBuildup {
		return <SiTypeBuildup> this._buildupsMap.get(this.selectedTypeId);
	}
...
//	}
	...
	handleError(error: SiEntryError) {
		for (let [fieldId, fieldError] of error.fieldErrors) {
			...
			const field = <SiField> this.selectedTypeBuildup.fieldMap.get(fieldId);
	...
	copy(): SiEntry {
		...
		for (const buildup of this._buildupsMap) {
