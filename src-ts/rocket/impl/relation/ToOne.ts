...
namespace Rocket.Impl.Relation {
	...
	class ToOneSelector {
		...
		get selectedEiId(): string {
			let eiId: string = this.jqInput.val().toString();
			if (eiId.length == 0) return null;
		...
		private selectEntry(eiId: string, identityString: string = null) {
